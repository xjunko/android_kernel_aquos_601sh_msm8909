...
#else
static DEVICE_ATTR(wcnss_mac_addr, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH,
	wcnss_wlan_macaddr_show, wcnss_wlan_macaddr_store);
#endif
/* [WLAN][SHARP] 2015.05.08 Change default SSID of Wi-Fi tethering End */
...
/* [WLAN][SHARP-EXT] 2016.2.18 Add "For NV switch" Start */
#ifdef SH_WIFI_CUSTOMIZE
void wcnss_get_nvfilename(char *pfileName)
{
	typedef struct {
		char version[4];
	...
	if (SMemCommAdrP) {
		...
		if (-1 <= val && val <= 1) {
			...
			pr_err("%s: NV level for 2.4GHz has invalid value(%d).\n", __FUNCTION__, val);
		}
	}
	else {
	...
	sprintf(pfileName, "wlan/prima/WCNSS_qcom_wlan_nv_%c.bin", pt_suffix_table[nv_switch_2_4G + 1]);
	pr_err("%s: NVFile is %s.\n", __FUNCTION__, pfileName);
}
EXPORT_SYMBOL(wcnss_get_nvfilename);
#endif
/* [WLAN][SHARP-EXT] 2016.2.18 Add "For NV switch" End */
...
/* [WLAN][SHARP-EXT] 2016.2.18 Add "For NV switch" Start */
	...
	wcnss_get_nvfilename(nvpath);
	ret = request_firmware(&nv, nvpath, dev);
...
/* [WLAN][SHARP] 2015.05.08 Change default SSID of Wi-Fi tethering Start */
	...
	SMemCommAdrP = sh_smem_get_common_address();
	if (SMemCommAdrP) {
		memcpy(&penv->wlan_nv_macAddr,
			   SMemCommAdrP->shdarea_WlanMacAddress,
			   sizeof(penv->wlan_nv_macAddr));
		...
		memset(&penv->wlan_nv_macAddr, 0, sizeof(penv->wlan_nv_macAddr));
	}
#endif
/* [WLAN][SHARP] 2015.05.08 Change default SSID of Wi-Fi tethering End */
