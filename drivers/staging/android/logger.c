...
static unsigned long buf_log_main;
static unsigned long buf_log_events;
#endif
...
#ifdef CONFIG_SHLOG_SYSTEM_LOGCAT
...
static int __init logger_init(void)
{
	...
	if (unlikely(ret))
		goto out;
#ifdef CONFIG_SHLOG_SYSTEM_LOGCAT
	buf_log_main = (unsigned long)list_entry(log_list.prev, struct logger_log, logs)->buffer;
#endif
	...
	if (unlikely(ret))
		goto out;
#ifdef CONFIG_SHLOG_SYSTEM_LOGCAT
	buf_log_events = (unsigned long)list_entry(log_list.prev, struct logger_log, logs)->buffer;
#endif
...
{
	return (unsigned long)(buf_log_main);
}
unsigned long get_buf_log_events_addr(void)
{
	return (unsigned long)(buf_log_events);
}
#endif
