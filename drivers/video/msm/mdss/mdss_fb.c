...
void mdss_fb_set_backlight(struct msm_fb_data_type *mfd, u32 bkl_lvl)
{
	...
	if ((((mdss_fb_is_power_off(mfd) && mfd->dcm_state != DCM_ENTER)
		|| (!mfd->bl_updated && !panel_displayed)) && bkl_lvl && !IS_CALIB_MODE_BL(mfd)) ||
...
#ifdef CONFIG_SHDISP /*   CUST_ID_00016  */
static int mdss_fb_get_lut(struct fb_info *info, void __user *p)
{
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)info->par;
	struct mdp_read_lut_data lut_data;
	int rc;

	mdp3_ctrl_read_lut(mfd,&lut_data);

	rc = copy_to_user(p,&lut_data,sizeof(struct mdp_read_lut_data));
	if(rc)
		return -EFAULT;
...
static int mdss_fb_get_specified_lut(struct fb_info *info, void __user *p)
{
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)info->par;
	struct mdp_read_specified_lut_data specified_lut_data;
	int rc;

	rc = copy_from_user(&specified_lut_data,p,sizeof(struct mdp_read_specified_lut_data));
	if(rc)
		return -EFAULT;

	if (specified_lut_data.level >= MDP_LUT_SIZE)
		return -EINVAL;

	mdp3_ctrl_read_specified_lut(mfd,&specified_lut_data);

	rc = copy_to_user(p,&specified_lut_data,sizeof(struct mdp_read_specified_lut_data));
	if(rc)
		return -EFAULT;
...
#ifdef CONFIG_SHDISP /*   CUST_ID_00016  */
	case MSMFB_GET_LUT:
		ret = mdss_fb_get_lut(info, argp);
		break;

	case MSMFB_GET_SPECIFIED_LUT:
		ret = mdss_fb_get_specified_lut(info, argp);
		break;
#endif
