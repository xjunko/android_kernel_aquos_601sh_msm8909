...
#ifdef CONFIG_SHDISP /*   CUST_ID_00018  */
static int mdp3_csc_read_config(struct mdp3_session_data *session,
			struct mdp_csc_cfg_data *read_data)
{
	int i;
	u32 addr;
	mdp3_clk_enable(1, 0);
	addr = MDP3_REG_DMA_P_CSC_MV1;
/* COORDINATOR Qualcomm_PostCS10_PostCS_modem BUILDERR MODIFY start */
//	if (session->cc_vect_sel)
	if (session->dma->cc_vect_sel)
/* COORDINATOR Qualcomm_PostCS10_PostCS_modem BUILDERR MODIFY end */
		addr = MDP3_REG_DMA_P_CSC_MV2;
	for (i = 0; i < 9; i++) {
		read_data->csc_data.csc_mv[i] = MDP3_REG_READ(addr);
		addr += 4;
	}
...
/* COORDINATOR Qualcomm_PostCS10_PostCS_modem BUILDERR MODIFY end */
	...
	for (i = 0; i < 3; i++) {
		read_data->csc_data.csc_pre_bv[i] = MDP3_REG_READ(addr);
		addr += 4;
	}
...
/* COORDINATOR Qualcomm_PostCS10_PostCS_modem BUILDERR MODIFY end */
	...
	for (i = 0; i < 3; i++) {
		read_data->csc_data.csc_post_bv[i] = MDP3_REG_READ(addr);
		addr += 4;
	}
...
/* COORDINATOR Qualcomm_PostCS10_PostCS_modem BUILDERR MODIFY end */
	...
	for (i = 0; i < 6; i++) {
		read_data->csc_data.csc_pre_lv[i] = MDP3_REG_READ(addr);
		addr += 4;
	}
...
/* COORDINATOR Qualcomm_PostCS10_PostCS_modem BUILDERR MODIFY end */
	...
	for (i = 0; i < 6; i++) {
		read_data->csc_data.csc_post_lv[i] = MDP3_REG_READ(addr);
		addr += 4;
	}
	mdp3_clk_enable(0, 0);
	return 0;
					...
					void __user *argp)
{
	...
	case mdp_op_csc_cfg:
		...
		break;
#ifdef CONFIG_SHDISP /*   CUST_ID_00018  */
	case mdp_op_read_csc_cfg:
		ret = mdp3_csc_read_config(mdp3_session,&(mdp_pp.data.csc_read_cfg_data));
		break;
#endif
				...
				void __user *argp)
{
	...
	case MSMFB_HISTOGRAM:
		...
		if(!copy_from_user(&get_c0,&hist.c0[0],sizeof(u32))) {
			if (!ret && mdp3_session && mdp3_session->panel && hist.block == MDP_BLOCK_DMA_P) {
				width = mdp3_session->panel->panel_info.xres;
				height = mdp3_session->panel->panel_info.yres;
...
static void mdp_gamma_lut_setup(struct shdisp_gamma_lut *gamma_lut_diff,struct msm_fb_data_type *mfd)
{
	u32 addr, color;
	int i,rc;
	struct mdp3_dma *dma;
	...
	rc = mdp3_alloc_lut_buffer(mfd->pdev,(void **)&dma->gc_cmap);
	if (rc) {
	...
	for (i = 0; i < SHDISP_GAMMA_LUT_ENTRIES; i++) {
		if(0 == i) {
			dma->gc_cmap->red[i]    = gamma_lut_diff->r_data[i];
			dma->gc_cmap->green[i]  = gamma_lut_diff->g_data[i];
			dma->gc_cmap->blue[i]   = gamma_lut_diff->b_data[i];
		} else {
			dma->gc_cmap->red[i]    = gamma_lut_diff->r_data[i] + dma->gc_cmap->red[i-1];
			dma->gc_cmap->green[i]  = gamma_lut_diff->g_data[i] + dma->gc_cmap->green[i-1];
			dma->gc_cmap->blue[i]   = gamma_lut_diff->b_data[i] + dma->gc_cmap->blue[i-1];
		}
	}

	addr = MDP3_REG_DMA_P_CSC_LUT1;
	for (i = 0; i < SHDISP_GAMMA_LUT_ENTRIES; i++) {
		color =  dma->gc_cmap->green[i] & 0xff;
		color |= (dma->gc_cmap->red[i] & 0xff) << 8;
		color |= (dma->gc_cmap->blue[i] & 0xff) << 16;
...
static void mdp3_ctrl_gamma_init(struct msm_fb_data_type *mfd)
{
	mdp_gamma_lut_setup(shdisp_api_get_gamma_lut(),mfd);
	return;
}

static void mdp3_ctrl_gamma_update(void)
{
	u32 lut_config;

	lut_config = MDP3_REG_READ(MDP3_REG_DMA_P_COLOR_CORRECT_CONFIG);

	lut_config &= DMA_LUT_CONFIG_MASK;
	lut_config |= 7;

	MDP3_REG_WRITE(MDP3_REG_DMA_P_COLOR_CORRECT_CONFIG, lut_config);
	wmb();
}
#endif
...
int mdp3_ctrl_read_lut(struct msm_fb_data_type *mfd,struct mdp_read_lut_data *lut_data)
{
	int i;
	u32 addr;
	struct mdp3_session_data *mdp3_session = mfd->mdp.private1;
	...
	mdp3_clk_enable(1, 0);
	lut_sel = (mdp3_session->lut_sel + 1) % 2;
	addr = MDP3_REG_DMA_P_CSC_LUT1;
	if (lut_sel)
		addr = MDP3_REG_DMA_P_CSC_LUT2;

	for (i = 0; i < MDP_LUT_SIZE; i++) {
		reg = MDP3_REG_READ(addr);
		lut_data->g_data[i] = reg & 0xff;
		lut_data->r_data[i] = (reg >> 8) & 0xff;
		lut_data->b_data[i] = (reg >> 16) & 0xff;
		addr += 4;
	}
	mdp3_clk_enable(0, 0);
	return 0;
...
int mdp3_ctrl_read_specified_lut(struct msm_fb_data_type *mfd,struct mdp_read_specified_lut_data *specified_lut_data)
{
	u32 addr;
	struct mdp3_session_data *mdp3_session = mfd->mdp.private1;
	u32 reg,lut_sel;
	...
	addr += 4*specified_lut_data->level;
	reg = MDP3_REG_READ(addr);
	specified_lut_data->g_data = reg & 0xff;
	specified_lut_data->r_data = (reg >> 8) & 0xff;
	specified_lut_data->b_data = (reg >> 16) & 0xff;
	mdp3_clk_enable(0, 0);
...
#ifdef CONFIG_SHDISP /*CUST_ID_00033*/
	rc = mdp3_ctrl_res_req_clk(mfd, 1);//open clk for gamma LUT updata
	if (rc) {
		pr_err("fail to request mdp clk resource\n");
		goto init_done;
	}

	mdp3_ctrl_gamma_init(mfd);
	if(mfd->panel.type != MIPI_CMD_PANEL) {
		mdp3_ctrl_gamma_update();
	}

	rc = mdp3_ctrl_res_req_clk(mfd, 0);//close clk
	if (rc) {
		pr_err("fail to request mdp clk resource\n");
		goto init_done;
	}
