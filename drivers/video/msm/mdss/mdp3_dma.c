...
#ifdef CONFIG_SHDISP /* CUST_ID_00015 */
static unsigned char shdisp_post_flag = false;
#endif
#ifdef CONFIG_SHDISP /* CUST_ID_00022 */
static int pp_update_flag = 0;
#endif
...
static void mdp3_vsync_intr_handler(int type, void *arg)
{
	...
	if (mdss_fb_base_fps_low_mode()) {
		dma->fpslow_count = (dma->fpslow_count + 1) & 1;
		if (dma->fpslow_count) {
			pr_debug("%s: return....\n", __func__);
			return;
		}
...
#ifdef CONFIG_SHDISP /* CUST_ID_00024 */
void mdp3_video_transfer_ctrl(struct msm_fb_data_type *mfd,bool onoff)
{
