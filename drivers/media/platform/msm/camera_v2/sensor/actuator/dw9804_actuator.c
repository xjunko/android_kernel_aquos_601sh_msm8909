...
static int32_t dw9804_act_status_check(struct msm_actuator_ctrl_t *a_ctrl)
{
	int32_t rc = -EFAULT;
	uint16_t data;
	uint32_t addr;
	...
	while(1) {
		rc = a_ctrl->i2c_client.i2c_func_tbl->
			i2c_read(&a_ctrl->i2c_client, addr, &data,
	...
	struct msm_actuator_move_params_t *move_params)
{
	...
	dw9804_act_status_check(a_ctrl);
...
/* SHLOCAL_CAMERA_DRIVERS-> */
static int32_t dw9804_act_i2c_write(struct msm_actuator_ctrl_t *a_ctrl,
	void __user *argp)
{
...
//	a_ctrl->i2c_client.addr_type = MSM_CAMERA_I2C_WORD_ADDR;
	...
	if(cdata->addr == 3) {
		rc = a_ctrl->i2c_client.i2c_func_tbl->
			i2c_write(&a_ctrl->i2c_client, cdata->addr, data,
...
static int32_t dw9804_act_i2c_read(struct msm_actuator_ctrl_t *a_ctrl,
	void __user *argp)
{
	struct i2c_info_t *cdata = (struct i2c_info_t *)argp;
	int32_t rc = -EFAULT;
...
/* SHLOCAL_CAMERA_DRIVERS-> */
	...
	CDBG("%s:%d SHCFG_SET_I2C_DATA\n", __func__, __LINE__);
		rc = dw9804_act_i2c_write(a_ctrl, &cdata->cfg.i2c_info);
		if (rc < 0)
	...
	CDBG("%s:%d SHCFG_GET_I2C_DATA\n", __func__, __LINE__);
		rc = dw9804_act_i2c_read(a_ctrl, &cdata->cfg.i2c_info);
		if (rc < 0)
