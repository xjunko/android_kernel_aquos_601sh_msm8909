...
static int shflip_probe(struct platform_device *pdev)
{
	...
	pin_state = pinctrl_lookup_state(pin, "flip_det_active");
	error = pinctrl_select_state(pin, pin_state);
	if(error){
		SHFLIP_ERR_LOG("flip_det pinctrl_select_state()\n");
	}
...
static int shflip_remove(struct platform_device *pdev)
{
	...
	pin = devm_pinctrl_get(&(pdev->dev));
	pin_state = pinctrl_lookup_state(pin, "flip_det_suspend");
