...
static int __init tuner_init(void)
{
	...
	for(i=0; i<loop; i++){
		...
		if ( use_gpiono[i].no > PM8841_GPIO_BASE ) {
			...
			ret = gpio_request( qpnp_pin_map("pm8916-gpio", mpp_no),  use_gpiono[i].label );
		}
		/* others */
		else {
...
static void __exit tuner_cleanup(void)
{
	...
	for(i=0; i<loop; i++){
		...
		if ( use_gpiono[i].no > PM8841_GPIO_BASE ) {
			mpp_no = use_gpiono[i].no - PM8841_GPIO_BASE;
			gpio_free( qpnp_pin_map("pm8916-gpio", mpp_no) );
		}
		/* others */
		else {
	...
	for (i=0; i<loop; i++, p++) {
		...
		//	printk(KERN_DEBUG "of_get_named_gpio() -- gpio_direction_input [%d][%s] \n", portno, use_gpiono[i].label);
			...
			if ( p->no > PM8841_GPIO_BASE ) {
				...
				if (gpio_direction_input( qpnp_pin_map("pm8916-gpio", mpp_no) ) < 0) {
					/* Failed */
					errcnt ++;
		...
		//	printk(KERN_DEBUG "of_get_named_gpio() -- direction_output [%d][%s] \n", portno, use_gpiono[i].label);
			...
			if ( p->no > PM8841_GPIO_BASE ) {
				...
				if (gpio_direction_output( qpnp_pin_map("pm8916-gpio", mpp_no), p->out_val ) < 0) {
					/* Failed */
					errcnt ++;
	...
	for(i=0; i<loop; i++, p++){
		if (p->id == id){
			...
			if ( p->no > PM8841_GPIO_BASE ) {
				/* PMIC8841_MPP */
				mpp_no = p->no - PM8841_GPIO_BASE;
				portno = qpnp_pin_map("pm8916-gpio", mpp_no);
			}
			else {
				portno = p->no ;
			}
