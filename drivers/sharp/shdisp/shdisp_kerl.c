...
int shdisp_shutdown_in_progress = 0;
#ifdef CONFIG_SHDISP_PANEL_SUBDISPLAY
extern int shdisp_exc_bkl_param;
#endif
...
static struct shdisp_gamma_lut* shdisp_get_gamma_lut(void);
...
#if defined (CONFIG_ANDROID_ENGINEERING)
static void shdisp_dbg_convert_to_8bp_from_1bp(unsigned char * buf8bp, 
                                        unsigned char * buf1bp,
                                        int width1bp, int height1bp);
static void shdisp_dbg_subdisp_image_output(void);
#endif /* CONFIG_ANDROID_ENGINEERING */
#endif
...
#endif
#ifdef CONFIG_SHDISP_PANEL_SUBDISPLAY
static int shdisp_SQE_sub_disp_update(struct shdisp_sub_update *sub_update);
#endif
...
static void shdisp_get_boot_context(void)
{
    ...
    if (sh_smem_common == NULL) {
        ...
        if( shdisp_panel_API_check_upper_unit(testmode_gpio) == SHDISP_RESULT_SUCCESS ) {
        ...
        shdisp_kerl_ctx.boot_ctx.vcom = local_boot_context->vcom;
        shdisp_kerl_ctx.boot_ctx.vcom_low = local_boot_context->vcom_low;
        shdisp_kerl_ctx.boot_ctx.upper_unit_is_connected = local_boot_context->upper_unit_is_connected;
#ifndef SHDISP_NOT_SUPPORT_PSALS
        memcpy(&(shdisp_kerl_ctx.boot_ctx.photo_sensor_adj), &(local_boot_context->photo_sensor_adj), sizeof(struct shdisp_photo_sensor_adj));
#endif
#else
...
static void shdisp_context_initialize(int mode)
{
    ...
    if (mode == 0) {
        ...
        for (i = 0; i < SHDISP_GAMMA_LUT_ENTRIES; i++) {
            if (i == 0) {
                shdisp_kerl_ctx.boot_ctx.gamma_lut.r_data[i] = 0x00;
                shdisp_kerl_ctx.boot_ctx.gamma_lut.g_data[i] = 0x00;
                shdisp_kerl_ctx.boot_ctx.gamma_lut.b_data[i] = 0x00;
            } else {
                shdisp_kerl_ctx.boot_ctx.gamma_lut.r_data[i] = 0x10;
                shdisp_kerl_ctx.boot_ctx.gamma_lut.g_data[i] = 0x10;
                shdisp_kerl_ctx.boot_ctx.gamma_lut.b_data[i] = 0x10;
            }
        }
    } else {
#ifndef SHDISP_NOT_SUPPORT_PSALS
...
static int shdisp_set_upper_unit(int mode)
{
    shdisp_kerl_ctx.boot_ctx.upper_unit_is_connected = mode;
...
static unsigned short shdisp_get_vcom(void)
{
    return shdisp_kerl_ctx.boot_ctx.vcom;
}
...
static unsigned short shdisp_get_vcom_low(void)
{
    return shdisp_kerl_ctx.boot_ctx.vcom_low;
}
...
static unsigned short shdisp_get_vcom_nvram(void)
{
    return shdisp_kerl_ctx.boot_ctx.vcom_nvram;
}
...
static struct shdisp_lcddr_phy_gamma_reg* shdisp_get_lcddr_phy_gamma(void)
{
    return &shdisp_kerl_ctx.boot_ctx.lcddr_phy_gamma;
}

/* ------------------------------------------------------------------------- */
/* shdisp_get_gamma_lut                                                       */
/* ------------------------------------------------------------------------- */

static struct shdisp_gamma_lut* shdisp_get_gamma_lut(void)
{
    return &shdisp_kerl_ctx.boot_ctx.gamma_lut;
}
...
static int shdisp_ioctl_get_context(void __user *argp)
{
    ...
    memcpy(&(shdisp_user_ctx.gamma_lut), &(shdisp_kerl_ctx.boot_ctx.gamma_lut), sizeof(struct shdisp_gamma_lut));
    memcpy(&(shdisp_user_ctx.lcddr_rom_gamma), &(shdisp_kerl_ctx.boot_ctx.lcddr_phy_gamma), sizeof(struct shdisp_lcddr_phy_gamma_reg));
    shdisp_semaphore_start();
...
struct shdisp_sub_update val;
unsigned char shdisp_sub_disp_buf[SHDISP_SUBDISPLAY_PANEL_HEIGHT_MAX * SHDISP_SUBDISPLAY_PANEL_WIDTH_MAX];
/*---------------------------------------------------------------------------*/
...
static int shdisp_ioctl_sub_disp_update(void __user *argp)
{
    ...
    ret = copy_from_user(&val, argp, sizeof(struct shdisp_sub_update));
    if (ret != 0) {
        SHDISP_ERR("<RESULT_FAILURE> copy_from_user.\n");
        shdisp_semaphore_end(__func__);
        return ret;
    ...
    SHDISP_DEBUG("start_xps(%d) start_yps(%d) width(%d) height(%d) buf(0x%p).\n"
                                , val.start_xps, val.start_yps, val.width, val.height, val.buf);

    if ( val.start_xps > SHDISP_SUBDISPLAY_PANEL_WIDTH_MAX || 
         val.start_yps > SHDISP_SUBDISPLAY_PANEL_HEIGHT_MAX ||
         val.width > SHDISP_SUBDISPLAY_PANEL_WIDTH_MAX ||
         val.height > SHDISP_SUBDISPLAY_PANEL_HEIGHT_MAX ||
         (val.start_xps + val.width)  > SHDISP_SUBDISPLAY_PANEL_WIDTH_MAX ||
         (val.start_yps + val.height) > SHDISP_SUBDISPLAY_PANEL_HEIGHT_MAX ) {
        SHDISP_ERR("<INVALID_VALUE> start_xps(%d) start_yps(%d) width(%d) height(%d).\n"
                                    , val.start_xps, val.start_yps, val.width, val.height);
        shdisp_semaphore_end(__func__);
        return SHDISP_RESULT_FAILURE;
    }

    if (val.buf == NULL) {
        SHDISP_ERR("<NULL_POINTER> val.buf.\n");
        shdisp_semaphore_end(__func__);
        return SHDISP_RESULT_FAILURE;
    }

    ret = copy_from_user(shdisp_sub_disp_buf, val.buf, SHDISP_SUBDISPLAY_PANEL_HEIGHT_MAX * SHDISP_SUBDISPLAY_PANEL_WIDTH_MAX * sizeof(unsigned char));
    if (ret != 0) {
        SHDISP_ERR("<RESULT_FAILURE> copy_from_user(buf).\n");
        shdisp_semaphore_end(__func__);
        return ret;
    }

    val.buf = shdisp_sub_disp_buf;
...
#ifdef CONFIG_SHDISP_PANEL_SUBDISPLAY
    shdisp_exc_bkl_param = request.param;
#endif
    switch (request.mode) {
...
{
    int ret = 0;
    unsigned short x_start = sub_update->start_xps;
    unsigned short y_start = sub_update->start_yps;
    unsigned short width = sub_update->width;
    unsigned short height = sub_update->height;
    int i,j,bit;
...
#define SHDISP_PARAM_MAX (4)
    ...
    case SHDISP_DEBUG_SUBDISPLAY_WRITE_REGISTER:
        ...
        if (1 == param[0]) {
            shdisp_SYS_API_subdisplay_spi_command_set(wbuf[0],wbuf[1]);
        } else if (2 == param[0]){
            shdisp_SYS_API_subdisplay_spi_multi_command_set(wbuf[0],&wbuf[1],param[0]);
        } else {
    ...
    case SHDISP_DEBUG_SUBDISPLAY_CHANGE_DATA:
        ...
        for (i = 0; i < SHDISP_SUBDISPLAY_HEIGHT_BUFFER_SIZE; i++) {
            if((i & 0x01) == 0)
                memset(&data_buf[i*SHDISP_SUBDISPLAY_WIDTH_BUFFER_SIZE],0xAA
                                  ,SHDISP_SUBDISPLAY_WIDTH_BUFFER_SIZE);
            else
                memset(&data_buf[i*SHDISP_SUBDISPLAY_WIDTH_BUFFER_SIZE],0x55
                                  ,SHDISP_SUBDISPLAY_WIDTH_BUFFER_SIZE);
       }
    ...
    case SHDISP_DEBUG_EXC_LCD_BLK_OFF:
        ...
        break;
#if defined (CONFIG_ANDROID_ENGINEERING)
    case SHDISP_DEBUG_SUBDISP_IMG_UPDATE:
        shdisp_semaphore_start();
        shdisp_dbg_subdisp_image_output();
        shdisp_semaphore_end(__func__);
        break;
#endif /* CONFIG_ANDROID_ENGINEERING */
#endif
...
static void shdisp_dbg_info_output(int mode)
{
    ...
    case SHDISP_DEBUG_INFO_TYPE_BOOT:
        ...
        printk("[SHDISP] boot_ctx.ledc_status.ledc_req.red[0]      = %d.\n", (int)shdisp_kerl_ctx.boot_ctx.ledc_status.ledc_req.red[0]);
        printk("[SHDISP] boot_ctx.ledc_status.ledc_req.red[1]      = %d.\n", (int)shdisp_kerl_ctx.boot_ctx.ledc_status.ledc_req.red[1]);
        printk("[SHDISP] boot_ctx.ledc_status.ledc_req.green[0]    = %d.\n", (int)shdisp_kerl_ctx.boot_ctx.ledc_status.ledc_req.green[0]);
        printk("[SHDISP] boot_ctx.ledc_status.ledc_req.green[1]    = %d.\n", (int)shdisp_kerl_ctx.boot_ctx.ledc_status.ledc_req.green[1]);
        printk("[SHDISP] boot_ctx.ledc_status.ledc_req.blue[0]     = %d.\n", (int)shdisp_kerl_ctx.boot_ctx.ledc_status.ledc_req.blue[0]);
        printk("[SHDISP] boot_ctx.ledc_status.ledc_req.blue[1]     = %d.\n", (int)shdisp_kerl_ctx.boot_ctx.ledc_status.ledc_req.blue[1]);
        ...
        for(i = 0; i < SHDISP_LCDDR_PHY_GAMMA_BUF_MAX; i++) {
            printk("%03X,", shdisp_kerl_ctx.boot_ctx.lcddr_phy_gamma.buf[i]);
        }
        printk("\n");
        ...
        printk("[SHDISP] boot_ctx.gamma_lut.red                  = ");
        printk("\n    ");
        for(i = 0; i < SHDISP_GAMMA_LUT_ENTRIES; i++) {
           if (i % 16 == 0) {
        printk("\n    ");
        }
           printk("%02X,", shdisp_kerl_ctx.boot_ctx.gamma_lut.r_data[i]);
        }
        printk("\n");
        printk("[SHDISP] boot_ctx.gamma_lut.green                = ");
        printk("\n    ");
        for(i = 0; i < SHDISP_GAMMA_LUT_ENTRIES; i++) {
           if (i % 16 == 0) {
        printk("\n    ");
        }
           printk("%02X,", shdisp_kerl_ctx.boot_ctx.gamma_lut.g_data[i]);
        }
        printk("\n    ");

        printk("[SHDISP] boot_ctx.gamma_lut.blue                 = ");
                printk("\n    ");
        for(i = 0; i < SHDISP_GAMMA_LUT_ENTRIES; i++) {
            if (i % 16 == 0) {
                printk("\n    ");
            }
           printk("%02X,", shdisp_kerl_ctx.boot_ctx.gamma_lut.b_data[i]);
        }
                printk("\n    ");
    ...
    case SHDISP_DEBUG_INFO_TYPE_POWERON:
        tmp = shdisp_kerl_ctx.boot_ctx.ledc_status.ledc_is_exist;
        tmp = shdisp_kerl_ctx.boot_ctx.ledc_status.power_status;
        printk("[SHDISP] BOOT INFO ->>\n");
        printk("[SHDISP] sizeof(shdisp_kerl_ctx.boot_ctx)                = %d.\n", sizeof(shdisp_kerl_ctx.boot_ctx));
        printk("[SHDISP] boot_ctx.driver_is_initialized         = %d.\n", shdisp_kerl_ctx.boot_ctx.driver_is_initialized);
        printk("[SHDISP] boot_ctx.hw_handset                    = %d.\n", (int)shdisp_kerl_ctx.boot_ctx.hw_handset);
...
#if defined (CONFIG_ANDROID_ENGINEERING)
/* ------------------------------------------------------------------------- */
/* shdisp_dbg_convert_to_8bp_from_1bp                                           */
/* ------------------------------------------------------------------------- */
static void shdisp_dbg_convert_to_8bp_from_1bp(unsigned char * buf8bp, 
                                        unsigned char * buf1bp,
                                        int width1bp, int height1bp)
{
    unsigned char * dst = buf8bp;
    unsigned char * src = buf1bp;
    int w = 0;
    int h = 0;
    int b = 0;

    for (h = 0; h < height1bp; h++) {
        for(w = 0; w < width1bp; w++) {
            for (b = 7; b >= 0; b--) {
                *dst = (((*src) & (1<<b)) ? 0xff : 0x00);
                dst++;
            }
            src++;
        }
...
static void shdisp_dbg_subdisp_image_output(void)
{
    #define SHDISP_DBG_SUBDISP_IMG_FORMAT "8bp"
    #define SHDISP_DBG_SUBDISP_IMG_FILE   "/data/subdisp_img"

    unsigned char * temp8bpbuf = NULL;
    int temp8bplen = SHDISP_SUBDISPLAY_PANEL_WIDTH_MAX
                     * SHDISP_SUBDISPLAY_PANEL_WIDTH_MAX;
    struct file *fp = NULL;
    ssize_t res;
    mm_segment_t old_fs;
    unsigned char filepathstr[256];

    SHDISP_TRACE("in\n");

    temp8bpbuf = kmalloc(temp8bplen, GFP_KERNEL);

    if (!temp8bpbuf) {
        SHDISP_ERR("failed to allocate memory\n");
        return;
    }

    snprintf(filepathstr, sizeof(filepathstr), "%s_w%d_h%d_%s.raw", 
        SHDISP_DBG_SUBDISP_IMG_FILE, 
        SHDISP_SUBDISPLAY_PANEL_WIDTH_MAX, 
        SHDISP_SUBDISPLAY_PANEL_HEIGHT_MAX,
        SHDISP_DBG_SUBDISP_IMG_FORMAT);

    fp = filp_open(filepathstr, O_WRONLY | O_CREAT | O_TRUNC, 0660);
    if (IS_ERR_OR_NULL(fp)) {
        SHDISP_ERR("failed to open file(%s)\n", filepathstr);
        kfree(temp8bpbuf);
        return;
    }

    shdisp_dbg_convert_to_8bp_from_1bp(temp8bpbuf, buf_write[0], 
                SHDISP_SUBDISPLAY_WIDTH_BUFFER_SIZE, 
                SHDISP_SUBDISPLAY_HEIGHT_BUFFER_SIZE);

    old_fs = get_fs();
    set_fs(get_ds());
    res = fp->f_op->write(fp, temp8bpbuf, temp8bplen, &fp->f_pos);
    set_fs(old_fs);
...
static int __init shdisp_init(void)
{
...
#ifdef CONFIG_SHDISP_PANEL_SUBDISPLAY
    ret = shdisp_SYS_API_subdisplay_spi_init();
    if (ret) {
        goto shdisp_err_5;
    }
    ...
    shdisp_pm_init(&shdisp_kerl_ctx.boot_ctx);

    state_str.bdic_is_exist = shdisp_kerl_ctx.boot_ctx.bdic_is_exist;
    state_str.bdic_chipver  = shdisp_kerl_ctx.boot_ctx.bdic_chipver;
    state_str.bdic_clrvari_index
      = shdisp_bdic_API_TRI_LED_get_clrvari_index( shdisp_kerl_ctx.boot_ctx.handset_color);
#ifndef SHDISP_NOT_SUPPORT_PSALS
...
#ifdef SHDISP_NOT_SUPPORT_NO_OS
    shdisp_bdic_API_get_bdic_chipver(&shdisp_kerl_ctx.boot_ctx.bdic_chipver);
#endif  /* SHDISP_NOT_SUPPORT_NO_OS */
...
#ifdef CONFIG_SHDISP_PANEL_SUBDISPLAY
    shdisp_SYS_API_subdisplay_spi_exit();
shdisp_err_5:
#endif
