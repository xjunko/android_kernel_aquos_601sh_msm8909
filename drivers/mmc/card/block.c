...
#ifdef CONFIG_MMC_SD_NO_SDDET_CUST_SH
	if (host->card)
#else /* CONFIG_MMC_SD_NO_SDDET_CUST_SH */
	if (mmc_cd_check_status(host) == 1)
#endif /* CONFIG_MMC_SD_NO_SDDET_CUST_SH */
		status = 1;
	else
		status = 0;
						...
						u32 cmd, sh_mmc_batlog_err_type type)
{
	...
	if (!mmc_detection_status_check(host)) {
		ignore = 1;
#ifndef CONFIG_MMC_SD_NO_SDDET_CUST_SH
		if (sh_mmc_battlog_debug_mask)
...
static const struct mmc_protect_inf mmc_protect_part[] = {
	...
	{19,	MMC_PROTECT_READ | MMC_PROTECT_WRITE	},
	{20,	MMC_PROTECT_READ | MMC_PROTECT_WRITE	},
