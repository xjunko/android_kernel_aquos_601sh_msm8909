...
#ifndef CONFIG_SUPPORT_V5_1_EMMC_CUST_SH
...
#endif /* CONFIG_SUPPORT_V5_1_EMMC_CUST_SH */
	...
	struct mmc_card *oldcard)
{
	...
	if (!oldcard) {
		...
		if ((card->cid.manfid == CID_MANFID_TOSHIBA) &&
			!strncmp(card->cid.prod_name, "008G70", 6))
			emmc_default_phase = 10;
		else if ((card->cid.manfid == CID_MANFID_SAMSUNG) &&
			!strncmp(card->cid.prod_name, "FN12MB", 6))
			emmc_default_phase = 5;
		else
			pr_err("%s: %s: default phase update fails :%d\n",
				mmc_hostname(host), __func__, emmc_default_phase);
#endif /* CONFIG_HS200_ALL_PHASE_PASS_EMMC_CUST_SH */
	}
