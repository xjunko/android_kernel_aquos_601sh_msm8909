		...
		struct qpnp_vadc_result *adc_chan_result)
{
	...
	if (batt_psy) {
		...
		scale_voltage = ret.intval;
		adc_chan_result->physical = ret.intval;

		scale_voltage = scale_voltage * chan_properties->offset_gain_numerator;
		if (scale_voltage < 0) {
			negative_offset = 1;
			scale_voltage = -scale_voltage;
		}

		do_div(scale_voltage, chan_properties->offset_gain_denominator);

		if (negative_offset)
			scale_voltage = -scale_voltage;

		scale_voltage -= chan_properties->adc_graph[chan_properties->calib_type].dx;
		scale_voltage = scale_voltage * chan_properties->adc_graph[chan_properties->calib_type].dy;
		...
		do_div(scale_voltage, chan_properties->adc_graph[chan_properties->calib_type].dx);

		if (negative_offset)
			scale_voltage = -scale_voltage;

		scale_voltage += chan_properties->adc_graph[chan_properties->calib_type].adc_gnd;
		if (scale_voltage < QPNP_VADC_MIN_ADC_CODE)
			temp_adc_code = QPNP_VADC_MIN_ADC_CODE;
		else if (scale_voltage > QPNP_VADC_MAX_ADC_CODE)
			temp_adc_code = QPNP_VADC_MAX_ADC_CODE;
		else
			temp_adc_code = (int32_t)(scale_voltage & 0xFFFFFFFF);
	} else {
		pr_err("battery supply not found\n");
	}

#if VBATT_CALIB_TYPE == 0
		qpnp_adc_scale_vbatt_calib_type_0(temp_adc_code, adc_chan_result);
#else
		qpnp_adc_scale_vbatt_calib_type_1(temp_adc_code, chan_properties, adc_chan_result);
#endif
