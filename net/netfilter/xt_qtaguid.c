...
static bool qtaguid_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	struct sock *sk;
	uid_t sock_uid;
/* SHARP_EXTEND 2015.07.16 Start */
	uid_t tmp_uid;
/* SHARP_EXTEND 2015.07.16 End */
	bool res;
	/*
...
//	if (info->match & XT_QTAGUID_UID)
	if (info->match & XT_QTAGUID_UID) {
		...
		sock_tag_entry = get_sock_stat(sk);
		if (sock_tag_entry) {
			tmp_uid = get_uid_from_tag(sock_tag_entry->tag);
			MT_DEBUG("qtaguid[%d]: UID changed to tagged uid=%d\n", par->hooknum, tmp_uid);
		} else {
			tmp_uid = filp->f_cred->fsuid;
			MT_DEBUG("qtaguid[%d]: UID not changed, uid=%d\n", par->hooknum, tmp_uid);
		}
//		if ((filp->f_cred->fsuid >= info->uid_min &&
//		     filp->f_cred->fsuid <= info->uid_max) ^
		if ((tmp_uid >= info->uid_min &&
		     tmp_uid <= info->uid_max) ^
/* SHARP_EXTEND 2015.07.16 End */
