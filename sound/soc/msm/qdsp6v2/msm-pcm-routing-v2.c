...
int msm_routing_get_is_music_play(void)
{
	...
	if (a2dp_mode)
		music_type |= 1 << 3;
	if (test_bit(0, &msm_bedais[34].fe_sessions))
		music_type |= 1 << 0;
	if (test_bit(4, &msm_bedais[34].fe_sessions))
		music_type |= 1 << 1;
	if (test_bit(1, &msm_bedais[34].fe_sessions))
		music_type |= 1 << 2;
	if (test_bit(3, &msm_bedais[34].fe_sessions))
		music_type |= 1 << 4;
	spin_unlock_irqrestore(&spinlock, flags);
	return music_type;
}
