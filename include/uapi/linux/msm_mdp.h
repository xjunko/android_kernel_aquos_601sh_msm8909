...
#define MSMFB_GET_LUT                  _IOW(MSMFB_IOCTL_MAGIC, 171, struct mdp_read_lut_data)
#define MSMFB_GET_SPECIFIED_LUT        _IOW(MSMFB_IOCTL_MAGIC, 172, struct mdp_read_specified_lut_data)
#define MSMFB_PANEL_STATE_CHG_WAIT _IOWR( MSMFB_IOCTL_MAGIC, 173 , unsigned int )
#define MSMFB_CHANGE_BASE_FPS_LOW _IOW(MSMFB_IOCTL_MAGIC, 174, unsigned int)
#define MSMFB_MIPI_DSI_CHECK  _IOWR(MSMFB_IOCTL_MAGIC, 175, struct mdp_mipi_check_param)
...
struct mdp_read_lut_data {
	uint8_t r_data[MDP_LUT_SIZE];
	uint8_t g_data[MDP_LUT_SIZE];
	uint8_t b_data[MDP_LUT_SIZE];
};

struct mdp_read_specified_lut_data {
	uint16_t level;
	uint8_t r_data;
	uint8_t g_data;
	uint8_t b_data;
...
struct msmfb_mdp_pp {
	...
	union {
		...
		struct mdp_csc_cfg_data csc_read_cfg_data;
